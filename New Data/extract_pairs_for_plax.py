...
file_counter=0
WRITE_COUNT=1
f=open('/media/ghazal/New Volume/extracted_pairs_plax.txt',"w+")
f2=open('/media/ghazal/New Volume/plax_paths.txt',"r+")
...
for i in range(3159):
    arr=f2.readline()
    ind = find(arr, '/')
    name = arr[np.amax(ind) + 1:-5]
    dash_ind=np.amax(find(name,'_'))
    base_name=name[dash_ind+1:]
